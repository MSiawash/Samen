...
const FALLBACK_ACTIVITEITEN = [
  {
    ...
    huidigeLeden: [
      { id: 'l1', naam: 'Jan (u)', rol: 'beheerder', mood: '☀️' },
      { id: 'l2', naam: 'Kees de Boer', rol: 'lid', mood: '⛅' },
      { id: 'l3', naam: 'Annie Jansen', rol: 'lid', mood: null },
      { id: 'l4', naam: 'Piet Visser', rol: 'mede-beheerder', mood: '🌧️' },
    ],
  },
  {
    id: '2',
    naam: 'Familie de Vries',
    beschrijving: 'Onze familie-activiteit om bij te praten en foto\'s te delen.',
    huidigeRolGebruiker: 'mede-beheerder',
    wachtendeVerzoeken: [
      { id: 'w3', naam: 'Neef Sander' },
    ],
    huidigeLeden: [
      { id: 'l5', naam: 'Jan (u)', rol: 'mede-beheerder', mood: '☀️' },
      { id: 'l6', naam: 'Marie de Vries', rol: 'beheerder', mood: '☀️' },
      { id: 'l7', naam: 'Tom de Vries', rol: 'lid', mood: null },
    ],
  },
  {
    id: '3',
...
const FALLBACK_OPENBARE_ACTIVITEITEN = [
  {
    id: 'o1',
...
// --- Beheer Modal ---
function BeheerModal({ activiteit, zichtbaar, onSluit, onUpdate }) {
  if (!activiteit) return null;

  function keurGoed(verzoekId) {
    const nieuw = { ...activiteit };
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <ScrollView showsVerticalScrollIndicator={false}>
            <Text style={styles.modalTitel}>Beheer: {activiteit.naam}</Text>

            {/* Nieuwe Verzoeken */}
            <Text style={styles.sectieTitel}>Nieuwe Verzoeken</Text>
            {activiteit.wachtendeVerzoeken.length === 0 ? (
              <Text style={styles.geenItems}>Geen openstaande verzoeken.</Text>
            ) : (
              activiteit.wachtendeVerzoeken.map((v) => (
            ...
            {activiteit.huidigeLeden.map((lid) => (
              <View key={lid.id} style={styles.lidRij}>
                <View style={{ flex: 1 }}>
                  <View style={styles.lidNaamRij}>
                    <Text style={styles.lidNaam}>{lid.naam}</Text>
                    <Text style={styles.lidMood}>
                      {lid.mood || '⚪'}
                    </Text>
                  </View>
...
// --- Hoofdscherm ---
export default function ActiviteitenScreen() {
  const [actieveSubTab, setActieveSubTab] = useState('mijn');
  const [activiteiten, setActiviteiten] = useState([]);
  const [openbareActiviteiten, setOpenbareActiviteiten] = useState(FALLBACK_OPENBARE_ACTIVITEITEN);
  const [isLaden, setIsLaden] = useState(true);
  const [beheerActiviteit, setBeheerActiviteit] = useState(null);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!geladen) {
        geladen = true;
        setActiviteiten(FALLBACK_ACTIVITEITEN);
        setIsLaden(false);
      }
    }, 4000);
    ...
    let unsubscribe;
    try {
      const q = collection(db, 'activiteiten');
      unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        if (snapshot.empty) {
          ...
          const data = snapshot.docs.map((doc) => {
            ...
            return {
              id: doc.id,
              naam: d.titel || d.naam || 'Onbekend',
              beschrijving: d.beschrijving || '',
              tijd: d.tijd || '',
              locatie: d.locatie || '',
              icoon: d.icoon || '📅',
              huidigeRolGebruiker: d.huidigeRolGebruiker || 'lid',
              wachtendeVerzoeken: d.wachtendeVerzoeken || [],
              huidigeLeden: d.huidigeLeden || [],
            };
          });
        ...
        geladen = true;
        clearTimeout(timeout);
        console.warn('Firestore activiteiten fout:', error);
        setActiviteiten(FALLBACK_ACTIVITEITEN);
        setIsLaden(false);
      });
    } catch (e) {
  ...
  // Aanmaken states
  const [nieuweActiviteitNaam, setNieuweActiviteitNaam] = useState('');
  const [gegenereerdeCode, setGegenereerdeCode] = useState('');

  function openBeheer(activiteit) {
    setBeheerActiviteit(activiteit);
  }

  function sluitBeheer() {
    setBeheerActiviteit(null);
  }

  function updateActiviteit(bijgewerkt) {
    setActiviteiten((prev) => prev.map((c) => (c.id === bijgewerkt.id ? bijgewerkt : c)));
    setBeheerActiviteit(bijgewerkt);
  }

  function verzendVerzoek(activiteitId) {
    setOpenbareActiviteiten((prev) =>
      prev.map((c) => (c.id === activiteitId ? { ...c, lidStatus: 'verzonden' } : c))
    );
  }

  const gefilterdeActiviteiten = openbareActiviteiten.filter((c) =>
  ...
  if (isLaden) {
    return (
      <View style={styles.laadScherm}>
        ...
        <Text style={styles.laadTekst}>Activiteiten laden...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.scherm}>
      ...
      <View style={styles.toggleBar}>
        {[
          { key: 'mijn', label: 'Mijn activiteiten' },
          { key: 'ontdekken', label: 'Ontdekken' },
          { key: 'maken', label: 'Maken' },
        ].map((t) => (
          <TouchableOpacity
            key={t.key}
            style={[styles.toggleKnop, actieveSubTab === t.key && styles.toggleActief]}
            activeOpacity={0.7}
            onPress={() => setActieveSubTab(t.key)}
            accessible
            accessibilityRole="tab"
            accessibilityState={{ selected: actieveSubTab === t.key }}
          >
            <Text style={[styles.toggleTekst, actieveSubTab === t.key && styles.toggleTekstActief]}>
              {t.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      >
        {/* ========== TAB: MIJN ACTIVITEITEN ========== */}
        {actieveSubTab === 'mijn' && (
          <>
            {activiteiten.map((activiteit) => (
              <View key={activiteit.id} style={styles.card}>
                <View style={styles.cardHeader}>
                  <Text style={styles.cardNaam}>{activiteit.naam}</Text>
                  <RolLabel rol={activiteit.huidigeRolGebruiker} />
                </View>
                <Text style={styles.cardBeschrijving}>{activiteit.beschrijving}</Text>

                {(activiteit.huidigeRolGebruiker === 'beheerder' ||
                  activiteit.huidigeRolGebruiker === 'mede-beheerder') && (
                  <TouchableOpacity
                    style={styles.beheerKnop}
                    activeOpacity={0.7}
                    onPress={() => openBeheer(activiteit)}
                    accessible
                    accessibilityRole="button"
                    accessibilityLabel={`Beheer leden en verzoeken van ${activiteit.naam}`}
                  >
                    <Text style={styles.beheerKnopTekst}>Beheer Leden & Verzoeken</Text>
                    {activiteit.wachtendeVerzoeken.length > 0 && (
                      <View style={styles.badge}>
                        <Text style={styles.badgeTekst}>{activiteit.wachtendeVerzoeken.length}</Text>
                      </View>
                    )}
                  </TouchableOpacity>
                )}
        ...
        {/* ========== TAB: ONTDEKKEN ========== */}
        {actieveSubTab === 'ontdekken' && (
          <>
            <View style={styles.card}>
              <Text style={styles.sectieKaartTitel}>Activiteiten zoeken</Text>
              ...
              <TouchableOpacity
                ...
                accessible
                accessibilityRole="button"
                accessibilityLabel="Zoek activiteiten"
              >
                <Text style={styles.zoekKnopTekst}>Zoek Activiteiten</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.zoekContainer}>
              <TextInput
                ...
                accessible
                accessibilityLabel="Zoek een activiteit op naam"
              />
            </View>

            {/* Openbare activiteiten */}
            {gefilterdeActiviteiten.map((activiteit) => (
              <View key={activiteit.id} style={styles.card}>
                <Text style={styles.cardNaam}>{activiteit.naam}</Text>
                <Text style={styles.cardBeschrijving}>{activiteit.beschrijving}</Text>
                {activiteit.lidStatus === 'verzonden' ? (
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                    onPress={() => verzendVerzoek(activiteit.id)}
                    accessible
                    accessibilityRole="button"
                    accessibilityLabel={`Verzoek om deel te nemen aan ${activiteit.naam}`}
                  >
                    <Text style={styles.verzoekKnopTekst}>Verzoek om deel te nemen</Text>
                  </TouchableOpacity>
                )}
              </View>
            ))}
        ...
        {actieveSubTab === 'maken' && (
          <View style={styles.card}>
            <Text style={styles.sectieKaartTitel}>Nieuwe activiteit aanmaken</Text>
            <Text style={styles.nieuweActiviteitHint}>
              U wordt automatisch beheerder van deze nieuwe activiteit.
            </Text>
            ...
            {gegenereerdeCode ? (
              ...
              <View style={styles.succesBlok}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setGegenereerdeCode('');
                    setNieuweActiviteitNaam('');
                  }}
                  accessible
                  accessibilityRole="button"
                  accessibilityLabel="Nog een activiteit aanmaken"
                >
                  <Text style={styles.resetKnopTekst}>Nog een activiteit aanmaken</Text>
                </TouchableOpacity>
              </View>
            ) : (
              /* Invoer-weergave */
              <>
                <Text style={styles.veldLabel}>Naam van uw nieuwe activiteit</Text>
                <TextInput
                  style={styles.invoerVeld}
                  placeholder="bijv. Wandelclubje"
                  placeholderTextColor="#B0B0B0"
                  value={nieuweActiviteitNaam}
                  onChangeText={setNieuweActiviteitNaam}
                  accessible
                  accessibilityLabel="Naam van uw nieuwe activiteit"
                />

                <TouchableOpacity
                  style={[
                    styles.aanmaakKnop,
                    !nieuweActiviteitNaam.trim() && styles.knopDisabled,
                  ]}
                  activeOpacity={0.7}
                  disabled={!nieuweActiviteitNaam.trim()}
                  onPress={() => {
                    const code = genereerUniekeCode();
                    setGegenereerdeCode(code);
                    console.log('Activiteit aangemaakt:', nieuweActiviteitNaam, 'Code:', code);
                  }}
                  accessible
                  accessibilityRole="button"
                  accessibilityLabel="Maak activiteit aan"
                >
                  <Text style={styles.aanmaakKnopTekst}>Nieuwe Activiteit</Text>
                </TouchableOpacity>
              </>
            )}
          </View>
      ...
      {/* Beheer Modal */}
      <BeheerModal
        activiteit={beheerActiviteit}
        zichtbaar={beheerActiviteit !== null}
        onSluit={sluitBeheer}
        onUpdate={updateActiviteit}
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  // Verzoek knop (openbare activiteiten)
  verzoekKnop: {
    backgroundColor: TEAL,
    borderRadius: 12,
    paddingVertical: 14,
